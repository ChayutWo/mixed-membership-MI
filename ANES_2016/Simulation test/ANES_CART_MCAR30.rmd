...
for (n_way in 1:max_n_way) {
  ...
  for (i in 1:(dim(combinations)[2])) {
    variables = combinations[, i]
    if (any(missing_col %in% variables)) {
      true_pmf_temp = c(table(X_df[,variables]))
  ...
  output_list = calculate_statistics(imputation_list = imputation_list, n_way = n_way, missing_col= missing_col)
  ...
  # Make a plot of true pmf vs estimated pmf
