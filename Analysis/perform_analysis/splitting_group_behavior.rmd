...
for (var in var_list) {
  ...
  summary = rbind(summary, c(pmf_a, pmf_b, pmf_base))
}
colnames(summary) <- c('level = 1', 'level = 2', 'level = 3', 
                       'level = 1', 'level = 2', 'level = 3',
                       'level = 1', 'level = 2', 'level = 3')
rownames(summary) <- var_list
caption = paste(" group a: ", group_a, " size = ", cluster_size_a, 
                ", group b: ", group_b, " size = ", cluster_size_b, 
                ", group base: ", group_base, " size = ", cluster_size_base, sep = '')
...
for (var in var_list) {
  ...
  phi = t(PHI_2[[strtoi(idx)]][group_base,,]) # matrix of size samples x # levels
  pmf_base =apply(phi, MARGIN=2, mean)
  summary = rbind(summary, c(pmf_merge, pmf_base))
}
colnames(summary) <- c('level = 1', 'level = 2', 'level = 3', 
                       'level = 1', 'level = 2', 'level = 3')
rownames(summary) <- var_list
caption = paste(" group a+b, size = ", mean(w_total), 
                ", group base: ", group_base, " size = ", cluster_size_base, sep = '')
xtable(summary, digits = 2, caption = caption)
```
