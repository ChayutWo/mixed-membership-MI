...
##### Mixed Membership Model for MI - Simulation example
rm(list=ls())
#library(knitr)
#library(dplyr)
#library(arm)
...
for (trial in 1:Mon) {
  ...
  #NEW VERSION
  for (k in 1:K){
    nik[,k] = rowSums(z_temp==k) #MOVE HERE!
    if(sum(nik[,k]>0)>0){
      ...
      #prob_k <- lapply( 1:length(non_zero_nik),
      #                  function(x) (alpha0_temp*beta_temp[k])/(alpha0_temp*beta_temp[k] + (1:non_zero_nik[x]) -1) )
      #sik[non_zero_index,k] <- unlist(lapply(prob_k, function(x) sum(rbernoulli(length(x), x))))
    }
  }
  t_temp = rbeta(N,alpha0_temp,rowSums(nik)) #MOVE HERE!
